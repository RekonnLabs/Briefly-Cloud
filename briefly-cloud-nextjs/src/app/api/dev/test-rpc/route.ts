...
export async function GET() {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('app.save_oauth_token', testData)
    ...
    const { data: retrievedData, error: getError } = await supabaseAdmin.rpc('app.get_oauth_token', {
      p_user_id: user.id,
      p_provider: 'google'
    })
