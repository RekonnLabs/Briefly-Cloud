-- ============================================================================
-- FINAL CORRECT SQL MIGRATION
-- Based on actual app.profiles table structure from Supabase Table Editor
-- Date: 2024-10-20
-- 
-- Columns in app.profiles:
-- - id (uuid, NOT NULL)
-- - email (text, NOT NULL)
-- - full_name (text, YES)
-- - avatar_url (text, YES)
-- - plan (text, NOT NULL)
-- - created_at (timestamp with time zone, NOT NULL)
-- - updated_at (timestamp with time zone, NOT NULL)
-- ============================================================================

-- Run this in Supabase SQL Editor:
-- https://supabase.com/dashboard/project/aeeumarwdxepqibjbkaf/sql/new

-- ============================================================================
-- Migration 001: Fix public.v_user_access View
-- ============================================================================

CREATE OR REPLACE VIEW public.v_user_access AS
SELECT 
  p.id                                                      AS user_id,
  p.email                                                   AS email,
  p.plan                                                    AS plan_tier,
  'active'                                                  AS plan_status,
  NULL                                                      AS trial_ends_at,
  FALSE                                                     AS trial_active,
  (p.plan IN ('pro', 'pro_byok'))                          AS paid_active
FROM app.profiles p;

-- Ensure proper grants (idempotent)
GRANT USAGE ON SCHEMA public TO authenticated, anon;
GRANT SELECT ON public.v_user_access TO authenticated, anon;

-- Add comment for documentation
COMMENT ON VIEW public.v_user_access IS 
  'Canonical view for user plan access. Sources from app.profiles. '
  'Used by /api/plan/status and storage OAuth gates. '
  'Updated 2024-10-20 to use correct column names (id not uuid).';

-- ============================================================================
-- Verification: Check the view works
-- ============================================================================

SELECT 
  user_id, 
  email, 
  plan_tier, 
  trial_active, 
  paid_active 
FROM public.v_user_access;

-- Expected result: Should show rekonnlabs@gmail.com with plan='free', paid_active=false

-- ============================================================================
-- Migration 002: Verify OAuth Token Infrastructure
-- ============================================================================

-- Check if oauth_tokens table exists
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema = 'private' AND table_name = 'oauth_tokens'
  ) THEN
    RAISE NOTICE 'SUCCESS: private.oauth_tokens table exists';
  ELSE
    RAISE NOTICE 'INFO: private.oauth_tokens table does not exist (this is OK if using Apideck only)';
  END IF;
END $$;

-- Check if RPC functions exist
DO $$
DECLARE
  save_token_exists boolean;
  get_token_exists boolean;
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'save_oauth_token'
  ) INTO save_token_exists;
  
  SELECT EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'get_oauth_token'
  ) INTO get_token_exists;
  
  IF save_token_exists THEN
    RAISE NOTICE 'SUCCESS: public.save_oauth_token function exists';
  ELSE
    RAISE NOTICE 'INFO: public.save_oauth_token function does not exist (OK if using Apideck)';
  END IF;
  
  IF get_token_exists THEN
    RAISE NOTICE 'SUCCESS: public.get_oauth_token function exists';
  ELSE
    RAISE NOTICE 'INFO: public.get_oauth_token function does not exist (OK if using Apideck)';
  END IF;
END $$;

-- ============================================================================
-- Migration 003: Verify pgvector Extension
-- ============================================================================

-- Check if pgvector extension exists
SELECT 
  CASE 
    WHEN EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') 
    THEN 'SUCCESS: pgvector extension is installed'
    ELSE 'ERROR: pgvector extension is NOT installed'
  END AS pgvector_status;

-- Create test function for pgvector (idempotent)
CREATE OR REPLACE FUNCTION public.test_pgvector_extension()
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  extension_exists boolean;
  test_vector vector(3);
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM pg_extension WHERE extname = 'vector'
  ) INTO extension_exists;
  
  IF NOT extension_exists THEN
    RAISE EXCEPTION 'pgvector extension is not installed';
  END IF;
  
  test_vector := '[1,2,3]'::vector(3);
  RETURN true;
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'pgvector test failed: %', SQLERRM;
    RETURN false;
END;
$$;

GRANT EXECUTE ON FUNCTION public.test_pgvector_extension() TO authenticated, anon;

-- Test the function
SELECT public.test_pgvector_extension() AS pgvector_working;
-- Expected: true

-- ============================================================================
-- Final Verification Queries
-- ============================================================================

-- 1. Verify your user exists in the view
SELECT 
  'Your user record:' AS info,
  user_id,
  email,
  plan_tier,
  paid_active
FROM public.v_user_access
WHERE email = 'rekonnlabs@gmail.com';

-- 2. Check document_chunks table exists and has embedding column
SELECT 
  'document_chunks table structure:' AS info,
  column_name, 
  data_type 
FROM information_schema.columns 
WHERE table_schema = 'app' 
  AND table_name = 'document_chunks'
ORDER BY ordinal_position;

-- 3. Check files table exists
SELECT 
  'files table structure:' AS info,
  column_name, 
  data_type 
FROM information_schema.columns 
WHERE table_schema = 'app' 
  AND table_name = 'files'
ORDER BY ordinal_position;

-- ============================================================================
-- DONE!
-- ============================================================================
-- If all queries above ran successfully, the database is ready.
-- Next step: Test the application login and document upload.
-- ============================================================================

